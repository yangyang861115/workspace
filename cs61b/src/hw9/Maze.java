  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    if (horiz > 1) {
      ...
      for (i = 0; i < horiz - 1; i++) {
        for (j = 0; j < vert; j++) {
          vWalls[i][j] = true;
          //System.out.print("(" + i + ',' + j + ')' + vWalls[i][j]);
        }
        //System.out.println();
      }
    }
     ...
     **/
    int numElements = horiz * vert;
    DisjointSets set = new DisjointSets(numElements);
    ...
    while (k > 1) {
        randNum = randInt(k);
        tmp = walls[k - 1];
        walls[k - 1] = walls[randNum];
        walls[randNum] = tmp;
        k--;
    }
    // visit each wall in walls
    int wallType;
    for (k = 0; k < numWalls; k++){
        int wallNum = walls[k];
        if(wallNum < horiz * (vert -1)) {
            // wall belongs to hWalls 
            wallType = 0;
            i = wallNum % horiz;
            j = wallNum / horiz;
        } else {
            // wall belongs to vWalls 
            wallNum = wallNum - horiz * (vert -1) ;
            wallType = 1;
            j = wallNum % vert;
            i = wallNum / vert;
        }
        ...
        int root1 = set.find(num1);
        int root2 = set.find(num2);
        if (root1 != root2) {
