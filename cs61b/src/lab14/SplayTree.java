...
public class SplayTree implements Dictionary {
    ...
    private void rotateLeft(BinaryTreeNode node) {
        if (node == null || node.parent == null || node.parent.rightChild != node) {
    ...
    private void splayNode(BinaryTreeNode node) {
        ...
        while (node.parent != null) {
            ...
            if (exParent == root) {
                zig(node);
            } else if ((subtreeParent.leftChild == exParent && exParent.rightChild == node)
                    || (subtreeParent.rightChild == exParent && exParent.leftChild == node)) {
                zigZag(node);
            } else if ((subtreeParent.leftChild == exParent && exParent.leftChild == node)
                    || (subtreeParent.rightChild == exParent && exParent.rightChild == node)) {
                zigZig(node);
            }
        }
    ...
    private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
        ...
        } else if (key.compareTo(node.entry.key()) > 0) { // "key" > "node"'s
                                                          // key
            if (node.rightChild == null) {
                splayNode(node); // Splay the last node visited to the root.
                return null;
    ...
    public static void main(String[] args) {
        ...
        BinaryTreeNode testNode = tree.findHelper((Comparable) testEntry.key, tree.root);
