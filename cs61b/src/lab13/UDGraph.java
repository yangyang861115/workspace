...
/**
 * The UDGraph class represents an unweighted directed graph. Implemented with
 * an adjacency matrix.
 */

public class UDGraph {
    /**
     * adjMatrix references the adjacency matrix of the graph. vertices is the
     * number of vertices in the graph. edges is the number of edges in the
     * graph.
     *
    ...
    /**
     * Returns a new UDGraph with the same vertices as "this" UDGraph. The new
     * graph has an edge (v, w) if and only if there is a path of length 2 from
     * v to w in "this" graph. *** DO NOT CHANGE "this" GRAPH!!! ***
     * 
    ...
    /**
     * Returns a new UDGraph with the same vertices as "this" UDGraph. The new
     * graph has an edge (v, w) if and only if there is a path of length
     * "length" from v to w in "this" graph.
     * 
     * @param length
     *            the length of paths used to construct the new graph.
     * @return the new UDGraph.
     */
    public UDGraph paths(final int length) {
        // Put your answer to Part II here.
        if (length == 2) {
            return length2Paths();
        }
        UDGraph newGraph = new UDGraph(vertices);
        UDGraph tmpGraph = paths(length - 1);
        for (int i = 0; i < vertices; i++) {
            for (int j = 0; j < vertices; j++) {
                for (int k = 0; k < vertices; k++) {
                    if (tmpGraph.hasEdge(i, j) && this.hasEdge(j, k)) {
                        newGraph.addEdge(i, k);
                    }
                }
            }
