...
public class BinaryTree implements Dictionary {
  ...
  private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
    // Replace the following line with your solution.
	  if(node == null || key.compareTo(node.entry.key) == 0){
		  return node;
  ...
  /**
   * min() searches for a BinaryTreeNode with the smallest key from a given node's right sub tree
   * return a BinaryTreeNode or null
   **/
    ...
    // Replace the following line with your solution.
      ...
      // cannot find the node
          ...
          // node has two children
              BinaryTreeNode nextNode = min(node);
              Entry replace = remove(nextNode.entry.key);
              node.entry = replace;
              size++;
          }else {
  ...
  public Entry remove2(Object key) {
      BinaryTreeNode node = findHelper((Comparable) key, root);
      if (node == null) {
          return null;
      }
      size--;
      BinaryTreeNode parent = node.parent, newNode = null;
      Entry removedEntry = node.entry;
      if (node.leftChild != null && node.rightChild != null) {
          // node has two children
              BinaryTreeNode nextNode = min(node);
              Entry replace = remove2(nextNode.entry.key);
              node.entry = replace;
              newNode = node;
              size++;
      } else if (node.leftChild != null || node.rightChild != null) {
